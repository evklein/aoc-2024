...
LIMIT = 6000 # Adjust for larger or more challenging datasets.

# Run raw
class Day6(Problem):
    def trace_guard_path(self, map):
    ...
    def PartB(self, input): # Nasty problem brute focing - TODO: rectangle search idea??
        ...
        for i, row in enumerate(original_map):
            for j, cell in enumerate(row):
                if cell != '#' and cell not in GUARD_ORIENTATIONS:
                    ...
                    new_map[i][j] = '#'
                    possible_loop_points += self.trace_guard_path(new_map).num_loop_points
